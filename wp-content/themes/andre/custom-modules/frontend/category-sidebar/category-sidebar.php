...
<div class="announcement-search-sidebar">
    ...
    <div><a href="" title="<?php _e('Announcement Map', THEME_TEXTDOMAIN); ?>"></a></div>
</div>
...
<div class="sidebar navbar-collapse collapse" id="bs-sidebar-navbar-collapse-1" aria-expanded="false" style="height: 1px;">
    <div class="widget-title hidden-xs">
        <h2><?php _e('CATEGORIAS', THEME_TEXTDOMAIN); ?></h2>
    </div>
            ...
            if (is_array($getCategories) && count($getCategories) > 0) :
                ...
                <ul class="nav navbar-nav">
                    ...
                    foreach ($getCategories as $eachcat) :
                        ...
                        ?>
                        <li>
                            <a style="<?php echo ($eachcat->slug == $getQueriedObject->slug) ? 'color: #fff;background: #4d5967;' : ''; ?>" class="<?php echo ($eachcat->slug == $getQueriedObject->slug) ? 'cat-active' : ''; ?>" data-toggle="collapse" href="<?php echo '#' . $eachcat->slug; ?>"><?php echo $eachcat->name; ?></a>
                            <div id="<?php echo $eachcat->slug; ?>" class="panel-collapse collapse">
                                <div class="panel-body">
                                    <ul class="nav navbar-nav">
                                        <li class="<?php echo ($eachcat->slug == $getQueriedObject->slug) ? 'cat-active' : ''; ?>"><a href="<?php echo get_term_link($eachcat); ?>"><?php _e('All products', THEME_TEXTDOMAIN); ?></a></li>
                                        <?php
                                        if (is_array($getSubCategories) && count($getSubCategories) > 0):
                                            foreach ($getSubCategories as $eachSubCategory):
                                                ?>
                                                <li class="<?php echo ($eachSubCategory->slug == $getQueriedObject->slug) ? 'cat-active' : ''; ?>"><a href="<?php echo get_term_link($eachSubCategory); ?>"><?php echo $eachSubCategory->name; ?></a></li>
                                                <?php
                                            endforeach;
                                        endif;
                                        ?>
